...
Code-Switching: The evaluation employed monolingual audio from Common Voice, where each sample contains a single language. This design choice enabled controlled language-specific performance analysis but precluded testing code-switching scenarios where speakers alternate between languages mid-utterance. Code-switching represents a common real-world phenomenon, particularly in multilingual communities, and poses unique challenges for both LID and ASR components. The LID→ASR pipeline must detect language transitions within utterances, while language-hinted mode cannot accommodate intra-utterance language changes. Specialized datasets (e.g., FLEURS with natural code-switching) or synthetic concatenation of multilingual samples would be required to systematically evaluate code-switching robustness. Future work should assess whether current architectures maintain accuracy during language transitions or exhibit degradation at code-switching boundaries.
